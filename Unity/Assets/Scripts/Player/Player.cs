...
public class Player : MonoBehaviour
{
    ...
    {
        // Calcular a dire��o apenas horizontalmente
        Vector3 direction = new Vector3(point.position.x - transform.position.x, 0, point.position.z - transform.position.z).normalized;

        // Criar a rota��o alvo apenas no eixo Y
        if (direction != Vector3.zero)
        {
            // LookAt n�o funciona apenas com o eixo Y (Y = 0)
            // Quaternion targetRotation = Quaternion.LookRotation(direction)
            Quaternion targetRotation = Quaternion.LookRotation(direction);

            // Manter a rota��o atual em X e Z e aplicar a nova rota��o em Y
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Waypoint"))
        {
            ...
            if (waypoint.waypath != null && !startingPathFound && waypoint.waypath.pathType == Waypath.PathType.START_LINE)
            {
                ...
                // Criar a rotaçãoo alvo apenas no eixo Y
                Quaternion targetRotation = Quaternion.LookRotation(direction);
